...
: m_nLayerCount( 0 ), mp_sListHead( NULL ), mp_sListTail( NULL ), m_nTop( -1 )
{
	...
	while ( pNode )
	{
		...
		else
		{
			...
			else
			{
				...
				switch( cMode )
				{
					case '*' : Top( )->SetUpperUnderLayer( pLayer ) ; break ;	// * => Top과 상호 상하위 계층으로 연결
					case '-' : Top( )->SetUnderLayer( pLayer ) ; break ;		// - => Top의 하위계층으로 연결
					case '+' : Top( )->SetUpperLayer( pLayer ) ; break ;		// + => Top의 상위계층으로 연결
				}
			}
		}
